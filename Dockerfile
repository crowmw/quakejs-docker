...
RUN apt-get update && apt-get install -y curl gpg git nginx supervisor
...
RUN apt-get purge curl gpg git -y && apt-get autoremove -y && apt-get clean && rm -rf /var/lib/apt/lists/*
...
# Use the entrypoint script to manage the server configuration files
